...
parser.add_argument('--root_dir', default='/media/zkk/duck/VL-PLM/datasets/coco', type=str,
...
def yolo2coco(arg):
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        # 读取图像的宽和高
        print(os.path.join(root_path, 'images/') + index)
        ...
        with open(os.path.join(originLabelsDir, txtFile), 'r') as fr:
            ...
            for label in labelList:
                ...
                width = max(0, x2 - x1)
                height = max(0, y2 - y1)
                print( int(label[0]),max(0, x2 - x1),max(0, y2 - y1))
