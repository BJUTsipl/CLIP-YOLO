...
from utils.loss import ComputeLoss, ComputeZSDLoss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Directories   存放最终的训练模型结果
    wdir = save_dir / 'weights'
    wdir.mkdir(parents=True, exist_ok=True)  # make dir
    last = wdir / 'last.pt'
    last_no_det = wdir / 'last_no_det.pt'
    best = wdir / 'best.pt'
    ...
    with open(opt.data) as f:
        data_dict = yaml.safe_load(f)  # data dict   读取文件列表
    ...
    #torch.save((pg0, pg1, pg2, temp_param, bg_param), 'debugging.pt')
    if opt.adam:
    ...
    if opt.zsd:
        if hyp['sim_func'] == 0:
            optimizer.add_param_group({'params': model.model[-1].sim_func.temp, 'lr': hyp['lr0'] * hyp['learnable_temp_scale']})
        elif hyp['sim_func'] == 1:
            optimizer.add_param_group({'params': model.model[-1].sim_func.cosine_scalar.shift, 'lr': hyp['lr0'] * hyp['learnable_shift_scale']})
            optimizer.add_param_group({'params': model.model[-1].sim_func.cosine_scalar.contrast, 'lr': hyp['lr0'] * hyp['learnable_contrast_scale']})
    ...
    if opt.linear_lr:
        ...
        lf = one_cycle(1, hyp['lrf'], intended_epochs)  # cosine 1->hyp['lrf']
    ...
    if pretrained and (not opt.skip_resume):
        # Optimizer
    ...
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size, gs, opt,  # testloader
                                       ...
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), annot_folder=opt.annot_folder)[0]
    ...
    if opt.zsd:
        hyp['normalizer_path'] = opt.normalizer_path
        hyp['initial_bg_path'] = opt.initial_bg_path
    ...
    results = (0, ) * (11 if opt.zsd else 7)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls, self_losses(if zsd))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni % accumulate == 0:
                ...
                if ema:
                    ema.update(model)
            #print(model.model[-1].bg[0], model.model[-1].sim_func.temp)
            # Print
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    Thread(target=plot_images, args=(imgs, targets[:, :6], paths, f, names), daemon=True).start()
                    if tb_writer:
        ...
        if rank in [-1, 0]:
            ...
            if opt.zsd:
                tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss', # train loss
                        'train/img_loss', 'train/text_loss', 'train/self_img_loss', 'train/self_text_loss', # zsd loss info
                        'metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95',
                        'val/box_loss', 'val/obj_loss', 'val/cls_loss', 'val/img_loss', 'val/text_loss', # val loss
                ...
                tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss', # train loss
                        ...
                        'x/lr0', 'x/lr1', 'x/lr2']  # params
            #print(list(mloss) + list(results) + lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='/media/zkk/ZSD-YOLO/data/coco/coco_zsd_2014_pretrain_65_15.yaml', help='data.yaml path')
    ...
    parser.add_argument('--annot-folder', type=str, default='labels', help='optional argument to change the path to folders relative to images')
    ...
    parser.add_argument('--initial-bg-path', default=None, type=str)
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'box': (1, 0.02, 0.4),  # box loss gain
                'cls': (1, 0.01, 0.4),  # cls loss gain
        ...
        meta = {
            ...
            'box': (1, 0.02, 0.4),
            'cls': (1, 0.01, 0.4),
            'cls_pw': (0, 0.631, 0.631),
            'contrast': (0, 4.0, 4.0),
            'degrees': (0, 0.373, 0.373),
            'diff_exp': (0, 0.5, 4.0),
            'fl_gamma': (0, 0.0, 0.0),
            'fliplr': (0, 0.5, 0.5),
            'flipud': (0, 0.00856, 0.00856),
            'hsv_h': (1, 0.0, 0.1),  # image HSV-Hue augmentation (fraction)
            'hsv_s': (1, 0.0, 0.9),  # image HSV-Saturation augmentation (fraction)
            'hsv_v': (1, 0.0, 0.9),  # image HSV-Value augmentation (fraction)
            'img_distill_weight': (1, 0.1, 10.0),
            'text_distill_weight': (1, 0.1, 10.0),
            'iou_t': (0.3, 0.1, 0.7),  # IoU training threshold
            ...
            'mosaic': (1, 0.0, 1.0),  # image mixup (probability)
            'normalize': (1, 0.0, 2.0),
            'obj': (1, 0.5, 4.0),  # obj loss gain (scale with pixels)
            'obj_pw': (1.0, 0.5, 2.0),  # obj BCELoss positive_weight
            'perspective': (0, 0.0, 0.0),
            'scale': (1, 0.0, 0.9),  # image scale (+/- gain)
            'self_img_loss_scalar': (1, 0.0, 2.0),
            'self_text_loss_scalar': (0, 0.0, 1.0),
            'shear': (1, 0.0, 10.0),  # image shear (+/- deg)
            'sim_func': (0, 0.0, 0.0),
            'learnable_background': (0, 0.0, 0.0),
            'learnable_temp_scale': (0.0, 0.0, 1.0),
            'temp': (1.0, 0.5, 4.0),
            'translate': (1, 0.0, 0.9),  # image translation (+/- fraction)
            'warmup_bias_lr': (0, 0.05, 0.05),
            'warmup_epochs': (0, 0.0, 1.0),
        ...
        for _ in range(1000):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                for i, k in enumerate(hyp.keys()):  # plt.hist(v.ravel(), 300)
                    hyp[k] = float(x[i + (11 if opt.zsd else 7)] * v[i])  # mutate
            ...
            #fix later
            print_mutation(hyp.copy(), results, yaml_file, opt.bucket, opt)
