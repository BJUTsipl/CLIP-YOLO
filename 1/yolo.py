...
'''
def sim_matrix(image_features, text_features, eps=1e-8):
...
class CosineScaler(nn.Module):
    def __init__(self, contrast=10.0, shift=6.0):
        super().__init__()
        self.contrast = nn.Parameter(torch.Tensor([contrast]), requires_grad=True)
        self.shift = nn.Parameter(torch.Tensor([shift]), requires_grad=True)
    
    def forward(self, x):
        return self.contrast * torch.log((0.5 * x + 0.5) / (1 - (0.5 * x + 0.5))) - self.shift
...
'''
class SoftmaxSim:
    def __init__(self, temp=0.02, eps=1e-8):
        super().__init__()
        self.temp = nn.Parameter(torch.Tensor([temp]), requires_grad=True)
        self.cosine_sim = DefaultCosine(eps)
    def __call__(self, cls_outputs, text_embeddings, favor=None):
        output = self.cosine_sim(cls_outputs, text_embeddings, favor) / self.temp
        output = torch.nn.functional.softmax(output, dim=-1)
        #if favor is not None:
        #    output *= favor.unsqueeze(0) if len(favor.shape) == 1 else favor
        return output        
'''    
...
class ZSDDetect(nn.Module):
    stride = None  # strides computed during build
    onnx_dynamic = False  # ONNX export parameter

    def __init__(self, nc=80, anchors=(), train_text_embedding_path=None, val_text_embedding_path=None, use_bg=False, ch=(), inplace=True):  # detection layer
        super(ZSDDetect, self).__init__()
        self.nc = nc  # number of classes
        assert train_text_embedding_path and val_text_embedding_path
        #MAKE SURE THESE EMBEDDINGS ARE IN THE SAME ORDER AS CLASSES FOUND IN DATA YAML
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, hyp=None):  # model, input channels, number of classes
        super().__init__()
        ...
        if isinstance(m, Detect) or isinstance(m, ZSDDetect):
            s = 256  # 2x min stride
...
def parse_model(d, ch, hyp=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv,RepConv, Bottleneck, GhostBottleneck, SPP,SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,SPPCSPC,
                 C3, C3TR, IntegratedTRC3,CBAM,CNeB,ConvNextBlock,BoT3,C3HB,Conv_SE,Conv_EMA,Conv_CA,Conv_CBAM,
                 ]:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, IntegratedTRC3,CBAM,CNeB,ConvNextBlock,BoT3,SPPCSPC,C3HB]:
